...
<x-layouts.site :seo-title="$seoTitle" :seo-description="$seoDescription ?? null" :seo-image="$seoImage ?? null">
    <div class="mx-auto max-w-5xl px-6 py-14">
        ...
        @if($center->cover_photo_url)
            <img src="{{ $center->cover_photo_url }}" alt="{{ $center->name }}" class="mt-6 h-72 w-full rounded-3xl object-cover sm:h-96">
        @endif
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                            @foreach($center->gallery_urls as $image)
                                <img src="{{ $image }}" alt="{{ $center->name }}" class="h-32 w-full rounded-xl object-cover">
                            @endforeach
                        </div>
                    </div>
                @endif

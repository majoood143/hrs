@php
    ...
    $query = \App\Models\ToolSalePost::query()
        ->visible()
        ->with(['city', 'country'])
    ...
    if ($selectedCategory) {
        $chips[] = ['label' => __('tools-for-sale.filter_category') . ': ' . __('tools-for-sale.categories.' . $selectedCategory), 'keys' => ['category']];
    }
    ...
    if ($countryId = request('country_id')) {
        if ($country = \App\Models\Country::find($countryId)) {
            $chips[] = ['label' => __('tools-for-sale.filter_country') . ': ' . $country->name, 'keys' => ['country_id']];
        }
    }
    if (request()->filled('price_max')) {
        $chips[] = ['label' => __('tools-for-sale.filter_price_max') . ': ' . \App\Models\SiteSetting::currency()['symbol'] . request('price_max'), 'keys' => ['price_max']];
    }
...
<div class="mx-auto max-w-7xl px-6 py-14" data-board>
    <div class="mx-auto max-w-2xl text-center" data-reveal>
        ...
        <h1 class="mt-2 font-display text-3xl font-semibold text-warm-900 sm:text-4xl">
            {{ $heading ?? __('tools-for-sale.board_title') }}
        </h1>
        @if($subheading ?? null)
            <p class="mt-3 text-warm-900/70">{{ $subheading }}</p>
        @endif
        <a href="{{ route('tools-for-sale.create') }}" class="btn-warm mt-5 inline-flex">{{ __('tools-for-sale.post_a_tool') }}</a>
    </div>
    ...
    <form method="GET" action="{{ request()->url() }}" data-board-filters class="mt-10">
        <div class="mt-3 flex justify-center gap-2 md:hidden">
            <button type="button" data-board-drawer-toggle class="btn-warm-outline !px-5 !py-2 text-sm">
                {{ __('tools-for-sale.filters_button') }}{{ $hasFilters ? ' · ' . count($chips) : '' }}
            </button>
        </div>
        ...
        <div class="mt-6 grid grid-cols-1 gap-4 rounded-3xl border border-warm-200/70 bg-white p-5 shadow-sm shadow-warm-900/5 md:grid-cols-6" data-board-panel>
            <div class="flex items-center justify-between md:hidden">
                <p class="font-display text-base font-semibold text-warm-900">{{ __('tools-for-sale.filters_button') }}</p>
                <button type="button" data-board-drawer-close class="text-sm font-semibold text-warm-600">{{ __('tools-for-sale.close') }}</button>
            </div>

            <div class="md:col-span-1">
                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-warm-700">{{ __('tools-for-sale.filter_category') }}</label>
                <select name="category" data-auto-submit class="w-full rounded-xl border border-warm-200 bg-warm-50 px-3 py-2.5 text-sm text-warm-900 focus:border-warm-500 focus:outline-none focus:ring-2 focus:ring-warm-300">
                    <option value="">{{ __('tools-for-sale.any') }}</option>
                    @foreach(\App\Models\ToolSalePost::CATEGORIES as $category)
                        <option value="{{ $category }}" @selected($selectedCategory === $category)>{{ __('tools-for-sale.categories.' . $category) }}</option>
                    @endforeach
                </select>
            </div>

            <div class="md:col-span-1">
                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-warm-700">{{ __('tools-for-sale.filter_condition') }}</label>
                <select name="condition" data-auto-submit class="w-full rounded-xl border border-warm-200 bg-warm-50 px-3 py-2.5 text-sm text-warm-900 focus:border-warm-500 focus:outline-none focus:ring-2 focus:ring-warm-300">
                    <option value="">{{ __('tools-for-sale.any') }}</option>
                    @foreach(\App\Models\ToolSalePost::CONDITIONS as $condition)
                        <option value="{{ $condition }}" @selected($selectedCondition === $condition)>{{ __('tools-for-sale.conditions.' . $condition) }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="md:col-span-1">
                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-warm-700">{{ __('tools-for-sale.filter_price_max') }}</label>
    ...
    @if($hasFilters)
        <div class="mt-5 flex flex-wrap items-center justify-center gap-2">
            ...
            <a href="{{ request()->url() }}" class="rounded-full border border-dashed border-warm-300 px-3 py-1.5 text-xs font-semibold text-warm-700 hover:bg-warm-100">
                {{ __('tools-for-sale.clear_filters') }}
            </a>
        </div>
    @endif

    <div class="mt-8 flex items-center justify-between text-sm text-warm-900/60">
        <span>{{ trans_choice('tools-for-sale.results_count', $tools->total(), ['count' => $tools->total()]) }}</span>

        <label class="flex items-center gap-2">
            <span class="hidden sm:inline">{{ __('tools-for-sale.sort_label') }}</span>
            <select form="board-sort" name="sort" onchange="document.getElementById('board-sort').submit()" class="rounded-lg border border-warm-200 bg-white px-3 py-1.5 text-sm font-semibold text-warm-800">
                <option value="newest" @selected($sort === 'newest')>{{ __('tools-for-sale.sort_newest') }}</option>
                <option value="price_asc" @selected($sort === 'price_asc')>{{ __('tools-for-sale.sort_price_asc') }}</option>
                <option value="price_desc" @selected($sort === 'price_desc')>{{ __('tools-for-sale.sort_price_desc') }}</option>
            </select>
        </label>
    ...
    @if($tools->isEmpty())
        <div class="mt-10 rounded-3xl border border-dashed border-warm-300 bg-white/60 px-6 py-16 text-center" data-reveal>
            <p class="font-display text-xl font-semibold text-warm-900">{{ __('tools-for-sale.no_results_title') }}</p>
            <p class="mt-2 text-warm-900/60">{{ __('tools-for-sale.no_results_body') }}</p>
            <a href="{{ route('tools-for-sale.create') }}" class="btn-warm mt-6 inline-flex">{{ __('tools-for-sale.post_a_tool') }}</a>
        </div>
    @else
                ...
                <article data-reveal-item class="card-warm overflow-hidden p-0">
                    ...
                    <div class="p-5">
                        <div class="flex flex-wrap gap-1.5">
                            <span class="rounded-full bg-warm-100 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-warm-700">{{ $tool->category_label }}</span>
                            <span class="rounded-full {{ $tool->condition === 'new' ? 'bg-emerald-100 text-emerald-700' : 'bg-warm-100 text-warm-700' }} px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide">{{ $tool->condition_label }}</span>
                        </div>

                        <p class="mt-3 font-display text-lg font-semibold text-warm-900">{{ $tool->name }}</p>

                        @if($tool->brand)
                            <p class="mt-0.5 text-sm text-warm-900/60">{{ $tool->brand }}</p>
                        @endif

@php
    $errorStep = 0;
    if ($errors->hasAny(['en_name', 'ar_name', 'specialty', 'years_experience'])) {
        $errorStep = 0;
...
<x-layouts.site :seo-title="$seoTitle">
    <div class="mx-auto max-w-3xl px-6 py-14 pb-28"
         ...
         data-step-label-template="{{ __('farriers.step_of', ['current' => ':current', 'total' => ':total']) }}"
        ...
        <div class="text-center" data-reveal>
            ...
            <h1 class="mt-2 font-display text-3xl font-semibold text-warm-900 sm:text-4xl">
                {{ __('farriers.post_page_title') }}
            </h1>
            <p class="mt-3 text-warm-900/70">{{ __('farriers.post_page_subtitle') }}</p>
        </div>
        ...
        @if($errors->any())
            <div class="mt-6 rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-700">
                <p class="font-semibold">{{ __('farriers.form_errors_title') }}</p>
        ...
        <form method="POST" action="{{ route('farriers.store') }}" class="mt-8" enctype="multipart/form-data" novalidate>
            @csrf

            {{-- Step 1: Farrier info --}}
            <div data-step>
                <h2 class="font-display text-xl font-semibold text-warm-900">{{ __('farriers.step_basic') }}</h2>
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.name_en') }}</label>
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.name_ar') }}</label>
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.specialty') }}</label>
                        <input type="text" name="specialty" value="{{ old('specialty') }}" class="w-full rounded-xl border border-warm-200 bg-warm-50 px-4 py-3 text-sm text-warm-900 focus:border-warm-500 focus:outline-none focus:ring-2 focus:ring-warm-300">
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.years_experience') }}</label>
                        <input type="number" name="years_experience" min="0" max="80" value="{{ old('years_experience') }}" class="w-full rounded-xl border border-warm-200 bg-warm-50 px-4 py-3 text-sm text-warm-900 focus:border-warm-500 focus:outline-none focus:ring-2 focus:ring-warm-300">
                    </div>
                </div>
            </div>
            ...
            <div data-step class="hidden">
                <h2 class="font-display text-xl font-semibold text-warm-900">{{ __('farriers.step_location') }}</h2>
                <div class="mt-5">
                    <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.city') }}</label>
                    <select name="city_id" required class="w-full rounded-xl border border-warm-200 bg-warm-50 px-4 py-3 text-sm text-warm-900 focus:border-warm-500 focus:outline-none focus:ring-2 focus:ring-warm-300">
                        <option value="">{{ __('farriers.select_city') }}</option>
            ...
            <div data-step class="hidden">
                <h2 class="font-display text-xl font-semibold text-warm-900">{{ __('farriers.step_details') }}</h2>
                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.cover_photo') }}</label>
                        ...
                        <p class="mt-1 text-xs text-warm-900/50">{{ __('farriers.photo_upload_hint') }}</p>
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.price') }}</label>
                        <div class="relative">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.contact_number') }}</label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.description') }} (EN)</label>
                    ...
                    <div class="sm:col-span-2">
                        <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.description') }} (AR)</label>
            ...
            <div data-step class="hidden">
                <h2 class="font-display text-xl font-semibold text-warm-900">{{ __('farriers.review_title') }}</h2>
                <dl class="mt-5 divide-y divide-warm-200 overflow-hidden rounded-2xl border border-warm-200 bg-white text-sm">
                    <div class="flex items-center justify-between px-5 py-3">
                        <dt class="text-warm-900/60">{{ __('farriers.name_en') }}</dt>
                        <dd data-review="name" class="font-semibold text-warm-900"></dd>
                    </div>
                    <div class="flex items-center justify-between px-5 py-3">
                        <dt class="text-warm-900/60">{{ __('farriers.city') }}</dt>
                        <dd data-review="city" class="font-semibold text-warm-900"></dd>
                    </div>
                    <div class="flex items-center justify-between px-5 py-3">
                        <dt class="text-warm-900/60">{{ __('farriers.price') }}</dt>
                        <dd data-review="price" class="font-semibold text-warm-900"></dd>
                    </div>
                    <div class="flex items-center justify-between px-5 py-3">
                        <dt class="text-warm-900/60">{{ __('farriers.contact_number') }}</dt>
                        <dd data-review="contact" class="font-semibold text-warm-900"></dd>
                    </div>
                </dl>

                <div class="mt-6">
                    <label class="mb-1 block text-sm font-semibold text-warm-800">{{ __('farriers.captcha_label') }}</label>
                    <div class="flex flex-wrap items-center gap-3">
                        <img
                            data-captcha-image
                            data-src="{{ route('farriers.captcha') }}"
                            src="{{ route('farriers.captcha') }}"
                            alt="{{ __('farriers.captcha_label') }}"
                            width="180" height="50"
                        ...
                        <button type="button" data-captcha-refresh class="text-xs font-semibold text-warm-600 underline underline-offset-2">
                            {{ __('farriers.captcha_refresh') }}
                        </button>
                    </div>
                    <input
                        type="text" name="captcha" required autocomplete="off"
                        placeholder="{{ __('farriers.captcha_placeholder') }}"
            ...
            <div class="sticky bottom-0 z-10 mt-8 flex items-center gap-3 border-t border-warm-200/70 bg-warm-50/95 py-4 backdrop-blur">
                <button type="button" data-back class="btn-warm-outline hidden">{{ __('farriers.back') }}</button>
                <div class="flex-1"></div>
                <button type="button" data-next class="btn-warm">{{ __('farriers.next') }}</button>
                <button type="submit" data-submit class="btn-warm hidden">{{ __('farriers.submit_post') }}</button>
            </div>
        </form>
    </div>
</x-layouts.site>

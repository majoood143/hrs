...
/**
 * A customer-facing fee added on top of a service's price. The rule that applies to an
 * order is the most specific active one: its form, else its service, else the global one.
 */
class ServiceFeeSetting extends Model
{
    ...
    /**
     * The fee rule for a service (and the form it is being ordered through), or null when
     * there is none. Form beats service beats global; the newest rule wins a tie.
     */
    ...
    {
        if ($this->fee_type === FeeType::Percentage) {
            return (int) round($priceBaisa * (float) $this->fee_value / 100, 0, PHP_ROUND_HALF_UP);
        }

        return (int) round((float) $this->fee_value * 1000);
    }

    public function getFormattedValueAttribute(): string
    {
        return $this->fee_type === FeeType::Percentage
            ? rtrim(rtrim(number_format((float) $this->fee_value, 3), '0'), '.').'%'
            : number_format((float) $this->fee_value, 3).' '.SiteSetting::currency()['code'];
    }

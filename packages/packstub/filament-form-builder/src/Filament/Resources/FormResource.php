    ...
    protected static function settingsSchema(): array
    {
        return [
            Section::make(__('packstub-form-builder::form-builder.sections.general'))
                ->columns(2)
                ->schema([
                    TranslatableInput::grid(fn (string $locale, array $meta): TextInput => TextInput::make("name.{$locale}")
                        ...
                        ->afterStateUpdated(function (?string $state, Get $get, Set $set, ?Model $record) use ($locale): void {
                            if ($locale === TranslatableInput::defaultLocale() && $record === null && blank($get('slug'))) {
                    ...
                    Toggle::make('is_active')
                        ...
                        ->default(true),
                    TranslatableInput::grid(fn (string $locale, array $meta): Textarea => Textarea::make("description.{$locale}")
            ...
            Section::make(__('packstub-form-builder::form-builder.sections.after_submit'))
                ->columns(2)
                ->schema([
                    TranslatableInput::grid(fn (string $locale, array $meta): TextInput => TextInput::make("submit_label.{$locale}")
                    ...
                    TextInput::make('redirect_url')
                        ...
                        ->maxLength(2048)
                        ->columnSpanFull(),
                    TranslatableInput::grid(fn (string $locale, array $meta): Textarea => Textarea::make("success_message.{$locale}")
